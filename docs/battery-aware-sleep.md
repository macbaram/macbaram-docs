...
The important detail is that MacBaram does not erase the user's intention. It keeps “I want this job to stay awake” separate from “it is safe to enforce that request right now.” When power conditions recover above the separate recovery boundary, the runtime can restore the original intention only while the current authorized control session and entitlement remain valid, instead of forcing the user to configure everything again.
